...
const list = async (req, res) => {
  ...
  try {
    ...
    const notes = await Promise.all(notesResult.map(async note => {
      const [tagsError, tagsResult] = await connectDB(
        'SELECT t.name FROM tag t INNER JOIN note_tag nt ON t.id = nt.tag_id WHERE nt.note_id = ?', 
        [note.id]
      );
      if (tagsError) {
        throw tagsError;
